...
function Property:UpdateOwner(data)
    ...
    local bank = targetPlayer.getAccount('bank')
...
RegisterNetEvent("ps-housing:server:buyFurniture", function(property_id, items, price)
    ...
    if price > PlayerData.getAccount("bank") and price > PlayerData.getMoney() then
...
lib.callback.register("ps-housing:cb:getPlayersWithAccess", function (source, property_id)
    ...
    for i = 1, #has_access do
        ...
        local Player = QBCore.Functions.GetPlayerByCitizenId(citizenid) or getESXOfflinePlayer(citizenid)
        local name = Player.firstName .. " " .. Player.firstName
        if Player then
...
lib.callback.register('ps-housing:cb:getPropertyInfo', function (source, property_id)
    ...
    if ownerCid then
        ownerPlayer = ESX.GetPlayerFromIdentifier(ownerCid) or getESXOfflinePlayer(ownerCid)
